...
class MCPServer {
    ...
    async handleAdvancedCall(args) {
        ...
        if (language && language !== 'English')
            briefText += ` Language: ${language}.`;
        if (date)
            briefText += ` Date: ${date}.`;
        if (time)
            briefText += ` Time: ${time}.`;
        if (location)
            briefText += ` Location: ${location}.`;
        if (constraints)
            briefText += ` Constraints: ${constraints}.`;
        if (fallback_options)
            briefText += ` Fallback options: ${fallback_options}.`;
        if (user_contact_return)
            briefText += ` Contact for follow-up: ${user_contact_return}.`;
        if (budget)
            briefText += ` Budget: ${budget}.`;
        if (urgency)
            briefText += ` Urgency: ${urgency}.`;
        if (industry)
            briefText += ` Industry: ${industry}.`;
        if (jurisdiction)
            briefText += ` Jurisdiction: ${jurisdiction}.`;
        if (allow_persuasion_white_lies)
            briefText += ` Persuasion allowed: yes.`;
        if (requires_formality)
    ...
    formatCallResult(result, toolType) {
        const lines = [
