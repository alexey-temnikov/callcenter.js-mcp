...
async function loadBaseConfig(configPath) {
...
function determineProvider(baseConfig, options) {
...
function mergeConfigurationSources(sources) {
    ...
    let mergedConfig = convertLegacyConfig(baseConfig);
...
function applyProviderProfile(baseConfig, profile) {
...
export function createSampleConfig(outputPath) {
    const sampleConfig = {
        sip: {
...
function loadEnvironmentOverrides() {
    ...
    if (process.env.SIP_USERNAME)
        overrides.username = process.env.SIP_USERNAME;
    if (process.env.SIP_PASSWORD)
        overrides.password = process.env.SIP_PASSWORD;
    if (process.env.SIP_SERVER_IP)
        overrides.serverIp = process.env.SIP_SERVER_IP;
    if (process.env.SIP_SERVER_PORT)
    ...
    if (process.env.STUN_SERVERS)
        overrides.stunServers = process.env.STUN_SERVERS.split(',');
    if (process.env.SIP_TRANSPORTS)
        overrides.preferredTransports = process.env.SIP_TRANSPORTS.split(',');
    // Session timers
    if (process.env.SESSION_TIMERS_ENABLED) {
        overrides.sessionTimers = {
            ...
            refresher: process.env.SESSION_REFRESHER || 'uac'
        };
    }
    return overrides;
}
...
export async function validateAndResolve(config) {
    const warnings = [];
    const suggestions = [];
    const errors = [];
...
function getConfigurationSuggestions(error) {
    const suggestions = [];
...
function getExampleConfigPaths() {
    return [
        'config.example.json',
