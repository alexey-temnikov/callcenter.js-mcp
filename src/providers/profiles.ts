...
export const PROVIDER_PROFILES: Record<string, SIPProviderProfile> = {
  'fritz-box': {
    name: 'AVM Fritz!Box',
...
export function getProviderProfile(providerId: string): SIPProviderProfile {
...
export function detectProviderFromDomain(domain: string): string | null {
...
export function validateProviderProfile(profile: SIPProviderProfile): string[] {
  const issues: string[] = [];
...
export function getProviderProfileByName(name: string): SIPProviderProfile | null {
  const providerId = Object.keys(PROVIDER_PROFILES).find(
    id => PROVIDER_PROFILES[id].name.toLowerCase() === name.toLowerCase()
  );
