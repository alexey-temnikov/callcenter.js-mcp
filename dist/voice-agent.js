...
export class VoiceAgent extends EventEmitter {
    ...
    batchTimer = null;
    BATCH_TIMEOUT_MS = 20; // Force send after 20ms for lower latency
    ...
    setupAudioBridge() {
        this.audioBridge.on("audioReceived", (audioData) => {
        ...
        this.openaiClient.onAudioReceived((audioData, responseId) => {
        ...
        this.openaiClient.on('aiEndCallDecision', (reason) => {
            this.aiEndCallReason = reason;
        });
        // Handle response with end_call completing generation - wait for audio to finish
        this.openaiClient.on('responseWithEndCallComplete', (responseId) => {
        ...
        this.openaiClient.on('responseGenerated', (responseId) => {
        ...
        this.openaiClient.on('getPlaybackPosition', (callback) => {
            if (this.audioBridge.isRunning()) {
        ...
        this.openaiClient.on('getPlayingResponseId', (callback) => {
            if (this.audioBridge.isRunning()) {
    ...
    async handleCallAnswered(event) {
        ...
        if (remoteRtpIp && remoteRtpPort) {
            getLogger().rtp.debug(`Setting up RTP connection to ${remoteRtpIp}:${remoteRtpPort}`);
